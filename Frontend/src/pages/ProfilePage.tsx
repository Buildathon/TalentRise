...
const ProfilePage = () => {
  ...
  // Utilidad para detectar si un error es de tipo Error
  const isError = (e: unknown): e is Error =>
    typeof e === "object" && e !== null && "message" in e;

  // Conectar MetaMask y crear instancia de contrato
    ...
    try {
      ...
      if (isError(err)) {
        if ((err as any).code === 4001) {
          ...
          setError("Error al conectar a MetaMask: " + err.message);
        }
      } else {
        setError("Error desconocido al conectar MetaMask.");
      }
    }
  };

  // Al conectar contrato, obtener datos
  useEffect(() => {
    if (!contract) return;
    ...
    const fetchContractData = async () => {
      try {
        ...
        if (isError(err)) {
          setError("Error al obtener datos del contrato: " + err.message);
        } else {
          setError("Error desconocido al leer contrato.");
        }
      }
    };
